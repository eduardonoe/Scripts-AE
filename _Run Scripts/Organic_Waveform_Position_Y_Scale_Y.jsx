/*
    ORGANIC WAVEFORM — POSITION Y + SCALE Y
    After Effects JSX
    Selecione os layers e execute o script.
*/
(function organicWaveformPositionScale() {
    app.beginUndoGroup("Organic Waveform - Position Y + Scale Y");
    ...
    var scaleExpression =
        "// ORGANIC WAVEFORM - SCALE Y\n" +
        "amp = 6;\n" +
        "speed = 0.5;\n" +
        "smoothness = 0.18;\n" +
        "phase = index * smoothness;\n" +
        "n1 = noise(time * speed + phase + 50);\n" +
        "n2 = noise(time * speed * 0.4 + phase * 0.7 + 200);\n" +
        "wave = n1 * 0.7 + n2 * 0.3;\n" +
        "[value[0], value[1] + wave * amp];";
    ...
    for (var i = 0; i < layers.length; i++) {
        try {
            ...
            if (scale && scale.canSetExpression)
                scale.expression = scaleExpression;
    ...
    app.endUndoGroup();
    alert("Organic Waveform — Position Y + Scale Y aplicado!\n\nLayers processados: " + applied);
})();
